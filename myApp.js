...
var createAndSavePerson = function(done) {
  var Lucas = new Person({name: "Lucas", age: 30, favoriteFoods: ["Tortilla", "Paella", "Poutine"]});
  
  Lucas.save((error,data) => {  
    if (error) return done(error)
    done(null, Lucas);
  });
};
...
var createManyPeople = function(arrayOfPeople, done) {
    Person.create(arrayOfPeople, (error, persons) => {
      if (error) return done(error)
      done(null, persons);
    });
...
var queryChain = function(done) {
 ...
 Person.find({favoriteFoods: foodToSearch}).sort({name: 'asc'}).limit(2).select('-age').exec((error, persons) => {
   console.log(persons);
   if (error) return done(error);
   done(null, persons);
 })
